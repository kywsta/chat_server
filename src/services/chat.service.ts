...
export class ChatService {
  ...
  async createChat(
    ...
    try {
      LoggerUtil.debug("Creating chat", {
        name,
        creatorId,
        memberIds,
        isGroup,
      ...
      LoggerUtil.info("Chat created successfully", {
        chatId: chat.id,
        name: chat.name,
      });
      return chat;
  ...
  async addMember(
    ...
    try {
      ...
      const existingMember = await this.chatMemberRepository.findByChatAndUser(
        chatId,
        userId
      );
      ...
      LoggerUtil.info("Member added to chat successfully", {
        chatId,
        userId,
        memberId: member.id,
      });
  ...
  async removeMember(chatId: string, userId: string): Promise<boolean> {
    try {
      ...
      const member = await this.chatMemberRepository.findByChatAndUser(
        chatId,
        userId
      );
      if (!member) {
        LoggerUtil.warn("Cannot remove member - member not found", {
          chatId,
          userId,
        });
        return false;
      ...
      const updated = await this.chatMemberRepository.update(member.id, {
        isActive: false,
      });

      if (updated) {
        LoggerUtil.info("Member removed from chat successfully", {
          chatId,
          userId,
          memberId: member.id,
        });
  ...
  async updateMemberRole(
    ...
    try {
      ...
      if (!member) {
        LoggerUtil.warn("Cannot update role - member not found", {
          chatId,
          userId,
          role,
        });
      ...
      const updatedMember = await this.chatMemberRepository.update(member.id, {
        role,
      });

      if (updatedMember) {
        LoggerUtil.info("Member role updated successfully", {
          chatId,
          userId,
          role,

import { ConditionalFilter, FilterOperator } from '../interfaces/database.interface';
...
export class FilterUtil {
   ...
   */
  static applyConditionalFilter<T>(entity: T, filter: ConditionalFilter): boolean {
   ...
   */
  static applyAllConditionalFilters<T>(entity: T, filters: ConditionalFilter[]): boolean {
    return filters.every(filter => this.applyConditionalFilter(entity, filter));
  }
   ...
   */
  private static startsWithComparison(entityValue: any, searchValue: string): boolean {
    if (typeof entityValue !== 'string') {
      return false;
    }
   ...
   */
  private static endsWithComparison(entityValue: any, searchValue: string): boolean {
    if (typeof entityValue !== 'string') {
      return false;
    }
